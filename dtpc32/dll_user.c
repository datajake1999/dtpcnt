...
int dtpcVolumeUp(int mod,int vol)
{
	int	status;
...
int dtpcVolumeDown(int mod,int vol)
{
	int	status;
	if((status = _dtpcDllEntry(mod,true)) != DLL_SUCCESS)
		return(_dtpcDllExit(status));
...
int dtpcVolumeSet(int mod,int vol)
{
	int	status;
	if((status = _dtpcDllEntry(mod,true)) != DLL_SUCCESS)
		return(_dtpcDllExit(status));
...
int dtpcFlushText(int mod)
{
	...
	if((status = _dtpcWaitDstat(30)) == DLL_FAILURE)
		return(_dtpcDllExit(status));
	outportb(DT_DMA,DMA_sync);
	outportb(DT_DMA,0);
	status = _dtpcWaitFlush();
	return(_dtpcDllExit(status));
}

int dtpcFlushChar(int mod,char c)
{
	int	status;
	if((status = _dtpcDllEntry(mod,true)) != DLL_SUCCESS)
		return(_dtpcDllExit(status));
	...
	if((status = _dtpcWaitDstat(30)) == DLL_FAILURE)
		return(_dtpcDllExit(status));
	outportb(DT_DMA,DMA_sync_char);
	outportb(DT_DMA,c);
	status = _dtpcWaitFlush();
	return(_dtpcDllExit(status));
}

int dtpcFlushSpeech(int mod)
{
	int	status;
	if((status = _dtpcDllEntry(mod,true)) != DLL_SUCCESS)
		return(_dtpcDllExit(status));
...
int dtpcImmdVoice(int mod,int voice)
{
	int	status;
	if((status = _dtpcDllEntry(mod,true)) != DLL_SUCCESS)
		return(_dtpcDllExit(status));
...
int dtpcImmdRate(int mod,int rate)
{
	int	status;
	if((status = _dtpcDllEntry(mod,true)) != DLL_SUCCESS)
		return(_dtpcDllExit(status));
...
int dtpcImmdRateDelta(int mod,int rate)
{
	int	status;
	if((status = _dtpcDllEntry(mod,true)) != DLL_SUCCESS)
		return(_dtpcDllExit(status));
...
int dtpcImmdPeriod(int mod,int pause)
{
	int	status;
	if((status = _dtpcDllEntry(mod,true)) != DLL_SUCCESS)
		return(_dtpcDllExit(status));
...
int dtpcImmdComma(int mod,int pause)
{
	int	status;
	if((status = _dtpcDllEntry(mod,true)) != DLL_SUCCESS)
		return(_dtpcDllExit(status));
...
int dtpcBuffFree(int mod, int *bytes)
{
	int	status;
	if((status = _dtpcDllEntry(mod,true)) != DLL_SUCCESS)
		return(_dtpcDllExit(status));
	status = _dtpcDoImmdGet(CMD_control+CTRL_buff_free,(unsigned int *)bytes);
	return(_dtpcDllExit(status));
}

int dtpcLastIndex(int mod, int *index)
{
	...
	if((unsigned int)curr_index != DTC.lastindex && DTC.module_status & STAT_index_valid)
		...
		{
		status = _dtpcDoImmdGet(CMD_control+CTRL_last_index,(unsigned int *)index);
...
int dtpcBuffUsed(int mod, int *bytes)
{
	int	status;
	if((status = _dtpcDllEntry(mod,true)) != DLL_SUCCESS)
		return(_dtpcDllExit(status));
	status = _dtpcDoImmdGet(CMD_control+CTRL_buff_used,(unsigned int *)bytes);
	return(_dtpcDllExit(status));
}

int dtpcFreeMem(int mod, int *paras)
{
	int	status;
	if((status = _dtpcDllEntry(mod,true)) != DLL_SUCCESS)
		return(_dtpcDllExit(status));
	status = _dtpcDoImmdGet(CMD_control+CTRL_free_mem,(unsigned int *)paras);
	return(_dtpcDllExit(status));
}

int dtpcIoPriority(int mod,int priority)
{
	int	status;
	if((status = _dtpcDllEntry(mod,true)) != DLL_SUCCESS)
		return(_dtpcDllExit(status));
...
int dtpcReset(int mod)
{
	int	status;
	if((status = _dtpcDllEntry(mod,false)) != DLL_SUCCESS)
		return(_dtpcDllExit(status));
	outportb(DT_CMD_LOW,0xff);
	outportb(DT_CMD_HIGH,0xff);
	outportb(DT_INT,0);
	DTC.id = ID_boot;
	return(_dtpcDllExit(DLL_SUCCESS));
}

int dtpcGetStatus(int mod,unsigned int *mod_status,int *number,int *mode)
{
	int	status;
...
int dtpcSendChar(int mod,char c)
{
	int	status;
	if((status = _dtpcDllEntry(mod,false)) != DLL_SUCCESS)
		return(_dtpcDllExit(status));
	...
	if((status = _dtpcWaitDstat(30)) == DLL_FAILURE)
		return(_dtpcDllExit(status));
	outportb(DT_DMA,DMA_single_in);
	outportb(DT_DMA,c);
	return(_dtpcDllExit(DLL_SUCCESS));
}


int dtpcGetChar(int mod,char *c)
{
	int	status;
	if((status = _dtpcDllEntry(mod,false)) != DLL_SUCCESS)
		return(_dtpcDllExit(status));
	...
	if((status = _dtpcWaitDstat(30)) == DLL_FAILURE)
		return(_dtpcDllExit(status));
	outportb(DT_DMA,DMA_single_out);
	outportb(DT_DMA,0);
	if((status = _dtpcWaitDstat(30)) == DLL_FAILURE)
		return(_dtpcDllExit(status));
	(*c) = inportb(DT_DMA);
...
int dtpcSendBuff(int mod,char *buff,int *count)
{
	...
	while((*count))
		...
		if((status = _dtpcWaitDstat(30)) == DLL_FAILURE)
			return(_dtpcDllExit(status));
		outportb(DT_DMA,DMA_buff_in);
		if(max_count == 256)
			outportb(DT_DMA,0);
		else
			outportb(DT_DMA,max_count);
		if((status = _dtpcWaitDstat(30)) == DLL_FAILURE)
			return(_dtpcDllExit(status));
		(*count) -= max_count;
		while(max_count--)
...
int dtpcGetBuff(int mod,char *buff,unsigned int *count)
{
	...
	while(max_count)
		...
		outportb(DT_DMA,DMA_buff_out);
		outportb(DT_DMA,i);
		if((status = _dtpcWaitDstat(30)) == DLL_FAILURE)
			return(_dtpcDllExit(status));
		j = inportb(DT_DMA) & 0xff;
		if(j == 0)
			break;
		(*count) += j;
		max_count -= j;
...
int dtpcSendDigitized(int mod,char *buff,int frames)
{
	int	status;
	int	i,j;
	...
	outportb(DT_DMA,DT_DIGITAL_IN);
	if((status = _dtpcWaitDstat(30)) == DLL_FAILURE)
		return(_dtpcDllExit(status));
	outportb(DT_DMA,(frames&0xff));
	outportb(DT_DMA,0);
	if((status = _dtpcWaitDstat(30)) == DLL_FAILURE)
		return(_dtpcDllExit(status));
	for(i=0;i<(int)frames;i++)
		{
		outportb(DT_DMA,SPC_DIGITIZED_DATA);
		outportb(DT_DMA,0);
		for(j=0;j<128;j++)
			outportb(DT_DMA,(*buff++));
		}
...
int dtpcSetup(int mod,HWND wnd)
{
	int	status;
	...
	if(DTS.settings_loaded == false)
		if((status=_dtpcLoadSettings()) != DLL_SUCCESS)
			return(_dtpcDllExit(status));
	DialogBox(dtpcInst, MAKEINTRESOURCE(DLG_CONFIG), wnd, (DLGPROC)_dtpcConfigDlg);
	return(_dtpcDllExit(DLL_SUCCESS));
}
...
int dtpcGetSetting(int mod,int setting,unsigned int *rvalue)
{
	int	status;
