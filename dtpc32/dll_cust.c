...
#define CTEXT   ((char *)&sp_names[curr_param*2][0])
#define CNAME   ((char *)&sp_names[curr_param*2+1][0])
#define CDEF    (sp_defaults[curr_param])

BOOL WINAPI _dtpcCustomDlg(HWND hWndDlg, UINT Message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
			...
			else switch(wParam)
	   {
				case    IDOK    :
						...
						temp = GetDlgItemInt(hWndDlg,ID_CUSTOM_VALUE,(int *)&ok_flag,FALSE);
						if(ok_flag)
							{
							if(temp < CMIN)
								{
				...
				case    ID_CUSTOM_CURRENT       :
					...
					if(DTC.loaded)
						...
						for(curr_param=0;curr_param<31;curr_param++)
							{
							temp = curr_param;
							_dtpcDoImmdGet(CMD_control+CTRL_speech+CTRL_SP_get_param,(unsigned int *)&temp);
							NVAL = temp;
							}
						_dtpcSetValDlg(hWndDlg,0);
						}
	    ...
	    case ID_TEST:
					...
					if(DTC.loaded)
						...
						for(curr_param=0;curr_param<31;curr_param++)
							{
							sprintf(temp_buff,"[:dv %s %d]",CNAME,NVAL);
							_dtpcSendInitBuff(temp_buff);
							}
						_dtpcSayTest();
						for(curr_param=0;curr_param<31;curr_param++)
							{
							sprintf(temp_buff,"[:dv %s %d]",CNAME,SVAL);
							_dtpcSendInitBuff(temp_buff);
							}
						curr_param = ok_flag;
						}
...
void _dtpcLoadVal()
{
	...
	for(curr_param=0;curr_param<31;curr_param++)
		{
		sprintf(temp_buff,"Val_%s",CNAME);
...
void _dtpcSaveVal()
{
	...
	for(curr_param=0;curr_param<31;curr_param++)
		{
		sprintf(temp_buff,"Val_%s",CNAME);
		sprintf(val_buff,"%d",CVAL);
		WritePrivateProfileString(dtpcSectionName,FTEMP_BUFF,(char *)&val_buff[0],dtpcIniFile);
		}
}
...
void _dtpcSetVal()
{
	for(curr_param=0;curr_param<31;curr_param++)
		{
		sprintf(temp_buff,"[:dv %s %d]",CNAME,CVAL);
		_dtpcSendInitBuff(temp_buff);
		}
}
	
int _dtpcSetValDlg(HWND hWndDlg,int param)
/*HWND hWndDlg; */
/*int param; */
{
 ...
 */
	...
	for(curr_param = 0;curr_param < 31; curr_param++)
		{
		sprintf(temp_buff,"%s: %d",CNAME,NVAL);
