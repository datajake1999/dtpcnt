...
int _dtpcLoadFile(char *fname, char *path)
{
	int     i,j,ext;
	int status;
...
int _dtpcLoadExe()
{
	...
	unsigned long    image_left,image_done,fix;
		...
		for(i=0;i<(int)header.relen;i++)
	      ...
	      fix = ((long)fixups[i].segment<<4)+(long)fixups[i].offset;
			if(fix >= image_done && fix < image_done+read_size)
				(*(unsigned short *)&read_buff[fix-image_done]) += seg_fix;
			}
...
int _dtpcLoadDic(int kind)
/*int kind; */
{

	int     i;
	unsigned int read_index,read_size,act_size;
	unsigned short *index_fix;
 ...
 */
   ...
   if(image_left>READ_BUFF_SIZE)
	...
	for(i=0;i<(int)header.relen;i++)
		{
	   fix = ((long)fixups[i].segment<<4)+(long)fixups[i].offset;
		(*(unsigned short *)&read_buff[fix]) += 0x40;
		}
 ...
 */
	...
	outportb((DTC.base+4),DMA_control);
	outportb((DTC.base+4),DT_LOAD_MEM);
	_dtpcWaitTicks(5);
	if(_dtpcWaitBit(5,STAT_dma_ready) == DLL_FAILURE)
		return(_dtpcErrorLoad(ERROR_dma_address));
	outportb((DTC.base+4),0x00);
	outportb((DTC.base+4),0x04);
	outportb((DTC.base+4),0x00);
	outportb((DTC.base+4),0x00);
	outportb((DTC.base+4),read_size&0xff);
	outportb((DTC.base+4),(read_size>>8)&0xff);
	_dtpcWaitTicks(5);
	if(_dtpcWaitBit(5,STAT_dma_ready) == DLL_FAILURE)
		return(_dtpcErrorLoad(ERROR_load_image));
	while(read_size--)
		outportb((DTC.base+4),*read_buff++);
 ...
 */
	...
	outportb((DTC.base+4),DMA_control);
	outportb((DTC.base+4),DT_START_TASK);
	_dtpcWaitTicks(5);
	if(_dtpcWaitBit(5,STAT_dma_ready) == DLL_FAILURE)
		return(_dtpcErrorLoad(ERROR_no_start_address));
	outportb((DTC.base+4),header.ipval&0xff);
	outportb((DTC.base+4),(header.ipval>>8)&0xff);
	outportb((DTC.base+4),header.csval&0xff);
	outportb((DTC.base+4),(header.csval>>8)&0xff);
...
int _dtpcFileResources()
{
	...
	read_buff = (char *)GlobalLock(hReadBuff);
	FileResources += 1;

	if((hFixUps = GlobalAlloc(GMEM_FIXED,MAX_FIXUPS*4)) == (HGLOBAL)NULL)
		return(_dtpcErrorLoad(ERROR_no_memory));
	fixups = (struct dos_reloc *)GlobalLock(hFixUps);
	FileResources += 1;

	if ((fp=_lopen(temp_buff,0)) == (HFILE)NULL)
...
int _dtpcErrorLoad(int status)
/*int   status; */
{
	_dtpcFreeResources();
	DTC.error_status = status;
	dtpcErrorStatus = status;
	sprintf(temp_buff,"Load Error: %s",ErrorStrings[status]);
