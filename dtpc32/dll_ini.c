...
unsigned  int _dtpcLoadSettings()
{
	


	memset((unsigned char *)&DTS.settings_loaded,0,sizeof(struct dectalk_settings));
...
unsigned  int _dtpcSaveSettings()
{
/*      int     i; */
...
int _dtpcSendSetString(int value, char *str[])
/*char *str[]; */
/*int value;   */
{
	int     i;
 ...
 */
	...
	if(stype == STRING_VALUE)
		{
		sprintf(temp_buff,"%s %d]",(unsigned char *)str[0],value);
 ...
 */
	...
	if(stype == STRING_DIRECT)
		return(_dtpcSendInitBuff( (char *)str[value] ));
 ...
 */
	...
	if(stype == STRING_FLAGS)
		...
		for(i=0;str[i*2];i++)
			{
			flag = (value & (1<<i));
			if(_dtpcSendInitBuff((char *)str[(i*2)+flag]) == DLL_FAILURE)
				return(DLL_FAILURE);
			}
		return(DLL_SUCCESS);
		}
...
int _dtpcSendInitBuff(char *buff)
/*char *buff; */
{
...
int _dtpcSayWelcome()
{
	if(DTS.welcome_string[0])
		{
		sprintf(FTEMP_BUFF,"%s%c",(char *)&DTS.welcome_string[0],0xb);
		return(_dtpcSendInitBuff(temp_buff));
		}
	return(DLL_SUCCESS);
}
...
int _dtpcSayTest()
{
	if(DTS.test_string[0])
		{
		sprintf(FTEMP_BUFF,"%s%c[:sync]",(char *)&DTS.test_string[0],0xb);
		return(_dtpcSendInitBuff(temp_buff));
		}
	return(DLL_SUCCESS);
}

int _dtpcSendInitChar(char c)
/*char c; */
{
	if(DTC.loaded)
		{
