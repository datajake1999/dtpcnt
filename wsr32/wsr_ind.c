...
/*	if(UserPauseControl)
		index_format = (char *)&pause_format_string[0];
	else																	 */
	index_format = (char *)&mark_format_string[0];
	wsrSetIndex();
}
...
void wsrSetIndex()
{
	...
	sprintf(index_string,index_format,si_head);
...
void wsrSetHighLight(int value,BOOL highlight)
{
	int	j,hx,hy,len;
	LPSTR psBuff;
