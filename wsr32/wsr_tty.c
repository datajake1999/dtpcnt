...
void	wsrSetupTTY(HDC hDC)
{
    RECT	rRect;

    GetClientRect(wsrWnd,&rRect);
    SetViewportOrgEx(hDC,0,rRect.bottom - ((MaxLines+1) * nStdioCharHeight), NULL);
    SetMapMode(hDC, MM_ANISOTROPIC);
    SetWindowExtEx(hDC,0,0,NULL);
...
LPSTR wsrGetTTYLine(short which)
{
    short pos;
...
void wsrPutTTY(HDC hDC,char *psOutput)
{
	int	i, j;
	char	*psBuff;
	int	iLinesOut;
...
void wsrHighLightText(int hx,int hy,int len,BOOL color)
{
	HDC hDC;
	LPSTR psLine;
