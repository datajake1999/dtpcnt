...
int wsrOpenFile()
{
	...
	if((wsrFile = _lopen(ofn.lpstrFile,0)) == 0)
		return(wsrFileError("Error reading file."));
	DialogBox(wsrInst, MAKEINTRESOURCE(DLG_TEXT_CONTROL), wsrWnd, (DLGPROC)wsrTextDlg);
	wsrClearTTY();
	return(SUCCESS);
}


int wsrReadFile(int mod)
{
	...
	if(UserReadControl)
		{
		wsrDlg = CreateDialog(wsrInst, MAKEINTRESOURCE(DLG_READ_CONTROL), wsrWnd, (DLGPROC)wsrReadControl);
		if(wsrDlg == NULL)
...
BOOL WINAPI wsrReadControl(HWND hWndDlg, UINT Message, WPARAM wParam, LPARAM lParam)
{
		...
		case WM_COMMAND:
			...
			if(wParam >= ID_VOICE_PAUL && wParam <= ID_VOICE_VAL)
            	...
            	case IDOK:
							...
							temp = GetDlgItemInt(hWndDlg,ID_VOLUME_VALUE,(BOOL *)&ok_flag,FALSE);
							if(ok_flag)
								{
								if(temp < MIN_VOLUME)
									{
							...
							temp = GetDlgItemInt(hWndDlg,ID_RATE_VALUE,(BOOL *)&ok_flag,FALSE);
							if(ok_flag)
								{
								if(temp < MIN_RATE)
									{
...
BOOL WINAPI wsrTextDlg(HWND hWndDlg, UINT Message, WPARAM wParam, LPARAM lParam)
{
